...
static hipStream_t flux_streams[MAX_GPUS][MAX_STREAMS];
                            ...
                            const int nz_per_chunk)
{
  ...
  if (hipMalloc((void**)&ec_d[gpu], ec_size) != hipSuccess)
  {
    printf("ERROR: out of memory for ec_d of %zd bytes on GPU %d\n", ec_size, gpu);
    exit(1);
  }
  hipMemcpy(ec_d[gpu], ec_h, ec_size, hipMemcpyHostToDevice);

  const size_t angle_size = num_angles * sizeof(double);
  if (hipMalloc((void**)&mu_d[gpu], angle_size) != hipSuccess)
  {
    printf("ERROR: out of memory for mu_d of %zd bytes on GPU %d\n", angle_size, gpu);
    exit(1);
  }
  hipMemcpy(mu_d[gpu], mu_h, angle_size, hipMemcpyHostToDevice);

  if (hipMalloc((void**)&eta_d[gpu], angle_size) != hipSuccess)
  {
    printf("ERROR: out of memory for eta_d of %zd bytes on GPU %d\n", angle_size, gpu);
    exit(1);
  }
  hipMemcpy(eta_d[gpu], eta_h, angle_size, hipMemcpyHostToDevice);

  if (hipMalloc((void**)&xi_d[gpu], angle_size) != hipSuccess)
  {
    printf("ERROR: out of memory for xi_d of %zd bytes on GPU %d\n", angle_size, gpu);
    exit(1);
  }
  hipMemcpy(xi_d[gpu], xi_h, angle_size, hipMemcpyHostToDevice);

  if (hipMalloc((void**)&w_d[gpu], angle_size) != hipSuccess)
  {
    printf("ERROR: out of memory for w_d of %zd bytes on GPU %d\n", angle_size, gpu);
    exit(1);
  }
  hipMemcpy(w_d[gpu], w_h, angle_size, hipMemcpyHostToDevice);

  const size_t flux_x_size = ny_per_chunk * nz_per_chunk * angle_size;
  for (int idx = 0; idx < MAX_STREAMS; idx++)
    if (hipMalloc((void**)&flux_x_d[gpu][idx], flux_x_size) != hipSuccess)
    {
  ...
  for (int idx = 0; idx < MAX_STREAMS; idx++)
    if (hipMalloc((void**)&flux_y_d[gpu][idx], flux_y_size) != hipSuccess)
    {
  ...
  for (int idx = 0; idx < MAX_STREAMS; idx++)
    if (hipMalloc((void**)&flux_z_d[gpu][idx], flux_z_size) != hipSuccess)
    {
  ...
  for (int corner = 0; corner < num_octants; corner++)
  {
    ...
    if (hipMalloc((void**)&wavefront_length_d[gpu][corner], 
          wavefront_count * sizeof(int)) != hipSuccess)
    {
      printf("ERROR: out of memory for wavefront length corner %d of %zd bytes on GPU %d\n",
             corner, wavefront_count * sizeof(int), gpu);
      exit(1);
    }
    hipMemcpy(wavefront_length_d[gpu][corner], wavefront_length_h, 
        wavefront_count * sizeof(int), hipMemcpyDeviceToHost);
    if (hipMalloc((void**)&wavefront_offset_d[gpu][corner], 
          wavefront_count * sizeof(int)) != hipSuccess)
    {
    ...
    hipMemcpy(wavefront_offset_d[gpu][corner], wavefront_offset_h, 
        wavefront_count * sizeof(int), hipMemcpyDeviceToHost);
    ...
    if (hipMalloc((void**)&wavefront_x_d[gpu][corner], offset * sizeof(int)) != hipSuccess)
    {
                        ...
                        const Rect<3> &subgrid_bounds, const int num_angles)
{
  ...
  switch (fa_xs.size())
  {
    case 1:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<1>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<1,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<1,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 2:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<2>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<2,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<2,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 3:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<3>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<3,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<3,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 4:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<4>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<4,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<4,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 5:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<5>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<5,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<5,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 6:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<6>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<6,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<6,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 7:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<7>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<7,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<7,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 8:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<8>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<8,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<8,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 9:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<9>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<9,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<9,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 10:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<10>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<10,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<10,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 11:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<11>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<11,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<11,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 12:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<12>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<12,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<12,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 13:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<13>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<13,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<13,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 14:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<14>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<14,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<14,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 15:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<15>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<15,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<15,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 16:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<16>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<16,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<16,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 24:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<24>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<24,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<24,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 32:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<32>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<32,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<32,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 40:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<40>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<40,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<40,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 48:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<48>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<48,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<48,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 56:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<56>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<56,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<56,
                                        AccessorWO<double,3>,3>(fa_dinv),
    ...
    case 64:
      {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_geometry_param<64>), dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                      AccessorArray<64,
                                        AccessorRO<double,3>,3>(fa_xs),
                                      AccessorArray<64,
                                        AccessorWO<double,3>,3>(fa_dinv),
...
{
  int numsm;
  hipDeviceGetAttribute(&numsm, hipDeviceAttributeMultiprocessorCount, gpu);
  ...
  hipOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm,
  ...
  for (int idx = 0; idx < MAX_STREAMS; idx++)
    if (hipMalloc((void**)&mutex_in_d[gpu][idx], grid_size*sizeof(int)) 
        != hipSuccess)
    {
  ...
  for (int idx = 0; idx < MAX_STREAMS; idx++)
    if (hipMalloc((void**)&mutex_out_d[gpu][idx], grid_size*sizeof(int)) 
        != hipSuccess)
    {
        ...
        case 1:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_dependent_sweep_with_fixup<1>,
        ...
        case 2:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_dependent_sweep_with_fixup<2>,
                grid, block, (void**)args, 0, stream);
            break;
          }
        default:
        ...
        case 1:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_independent_sweep_with_fixup<1>,
                grid, block, (void**)args, 0, stream);
            break;
          }
        case 2:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_independent_sweep_with_fixup<2>,
                grid, block, (void**)args, 0, stream);
            break;
          }
        default:
        ...
        case 1:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_dependent_sweep_without_fixup<1>,
                grid, block, (void**)args, 0, stream);
            break;
          }
        case 2:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_dependent_sweep_without_fixup<2>,
                grid, block, (void**)args, 0, stream);
            break;
          }
        default:
        ...
        case 1:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_independent_sweep_without_fixup<1>,
                grid, block, (void**)args, 0, stream);
            break;
          }
        case 2:
          {
            ...
            hipLaunchCooperativeKernel((const void*)gpu_time_independent_sweep_without_fixup<2>,
                grid, block, (void**)args, 0, stream);
            break;
          }
        default:
