                                    ...
                                    const AccessorWO<MomentQuad,3> fa_qtot)
{
  ...
  hipLaunchKernelGGL(gpu_inner_source_single_moment, dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                                                 fa_sxs, fa_flux0, 
                                                 fa_q2grp0, fa_qtot);
}
                                   ...
                                   const int num_moments, const int lma[4])
{
  ...
  dim3 block(gcd(x_range,32),gcd(y_range,4),gcd(z_range,4));
  dim3 grid(x_range/block.x, y_range/block.y, z_range/block.z);
  hipLaunchKernelGGL(gpu_inner_source_multi_moment, dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
                           ...
                           const double epsi)
{
  ...
  for (unsigned idx = 0; idx < fa_flux0.size(); idx++) {
    hipLaunchKernelGGL(gpu_inner_convergence, dim3(grid), dim3(block), 0, 0, subgrid_bounds.lo,
  ...
  hipLaunchKernelGGL(gpu_sum_inner_convergence, dim3(grid2), dim3(block2), 0, hipGetTaskStream(), buffer, result, bounds.hi[0]+1, expected);
}
