                         ...
                         const std::vector<AccessorWO<double,3> > fa_qo0)
{
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_flux0_outer_source<GROUPS,MAX_X*MAX_Y>), dim3(grid), dim3(block), 0, hipGetTaskStream(), 
                              subgrid_bounds.lo,
                         ...
                         const int num_groups, const int num_moments, const int lma[4])
{
  ...
  dim3 block(gcd(x_range,MAX_X), gcd(y_range,MAX_Y), GROUPS);
  dim3 grid(x_range/block.x, y_range/block.y, z_range);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(gpu_fluxm_outer_source<GROUPS,MAX_X*MAX_Y>), dim3(grid), dim3(block), 0, hipGetTaskStream(), 
                            subgrid_bounds.lo,
                            AccessorArray<GROUPS,
                              AccessorRO<MomentTriple,3>,3>(fa_fluxm),
                            AccessorArray<GROUPS,
                           ...
                           const double epsi)
{
  ...
  for (unsigned idx = 0; idx < fa_flux0.size(); idx++) {
    hipLaunchKernelGGL(gpu_outer_convergence, dim3(grid), dim3(block), 0, hipGetTaskStream(), subgrid_bounds.lo,
  ...
  hipLaunchKernelGGL(gpu_sum_outer_convergence, dim3(grid2), dim3(block2), 0, hipGetTaskStream(), buffer, result, bounds.hi[0]+1, expected);
}
